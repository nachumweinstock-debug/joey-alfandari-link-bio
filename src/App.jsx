...
const bioText =
  "Joey is a builder, creative operator, and storyteller with an eye for momentum. He is interested in sharp ideas, honest work, and projects that make people feel something.";
...
const links = [
  {
    ...
    icon: Instagram,
    note: "Instagram is ready. Swap this for Joey's final profile URL whenever needed.",
  },
  {
    label: "Projects",
    href: "#projects",
    icon: BriefcaseBusiness,
    note: "Projects placeholder selected. Add Joey's featured work here.",
  },
  {
    label: "Contact",
    href: "mailto:hello@example.com",
    icon: Mail,
    note: "Contact placeholder selected. Replace with Joey's real email or form link.",
  },
];
...
function LinkCard({ href, icon: Icon, label }) {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="group flex min-h-[70px] items-center justify-between rounded-[8px] border border-neutral-900/15 bg-white/70 px-5 py-4 text-left shadow-[0_12px_35px_rgba(23,23,23,0.06)] backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-neutral-950 hover:bg-white hover:shadow-[0_18px_45px_rgba(23,23,23,0.12)] focus:outline-none focus:ring-4 focus:ring-orange-500/25"
    >
      <span className="flex items-center gap-4">
        <span className="flex size-11 shrink-0 items-center justify-center rounded-full bg-neutral-950 text-orange-500 transition duration-300 group-hover:rotate-[-8deg] group-hover:scale-105">
      ...
      {copied ? (
        <Check size={22} className="text-orange-600" />
      ) : (
        <ArrowUpRight
          size={22}
          className="text-neutral-950 transition duration-300 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-orange-600"
        />
      )}
    </button>
  );
...
export default function App() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#fbf5ea] text-neutral-950">
      <section className="relative flex min-h-screen items-center px-5 py-8 sm:px-8 lg:px-10">
        <div className="pointer-events-none absolute left-0 top-0 h-40 w-40 -translate-x-1/2 -translate-y-1/2 rounded-full bg-orange-500/20 blur-3xl" />
        <div className="pointer-events-none absolute bottom-12 right-0 h-48 w-48 translate-x-1/3 rounded-full bg-orange-500/15 blur-3xl" />
        ...
        <div className="mx-auto grid w-full max-w-5xl gap-10 lg:grid-cols-[1fr_340px] lg:items-center">
          ...
          <div className="animate-rise-in mx-auto w-full max-w-[300px] [animation-delay:120ms] lg:max-w-none">
            <div className="relative aspect-square rounded-[32px] border border-neutral-950/15 bg-white p-3 shadow-[0_24px_70px_rgba(23,23,23,0.15)]">
              <div className="absolute -right-4 -top-4 flex size-16 items-center justify-center rounded-full bg-orange-500 text-white shadow-[0_16px_35px_rgba(255,90,31,0.35)]">
              ...
              {profilePhotoUrl && photoReady ? (
                ...
                <div className="flex h-full w-full items-center justify-center rounded-[24px] bg-[linear-gradient(135deg,#171717_0%,#2b2b2b_48%,#ff5a1f_100%)] text-center">
      ...
      <section className="px-5 pb-20 sm:px-8 lg:px-10">
        <div className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[0.85fr_1.15fr]">
          ...
          <div className="animate-rise-in [animation-delay:320ms]">
            ...
            <div className="mt-5 rounded-[8px] border border-orange-600/25 bg-orange-500/10 p-4 text-sm font-semibold leading-6 text-neutral-800">
              <span className="text-orange-700">{selectedLink.label}:</span>{" "}
              {selectedLink.note}
